...
const SummaryItem = ({ summary, onSummaryDeleted }: SummaryItemProps) => {
  const { getToken } = useAuth();

  const deleteProject = async () => {
    if (
      confirm(`Are you sure you want to delete "${summary.id}"?`) === false
    ) {
      return;
    }
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/summaries/${summary.id}`,
        {
          method: "DELETE",
          headers: {
